            ...
            hbox:
                ...
                vbox:
                    vbox:
                        ...
                        textbutton _("Активировать режим"):
                            action ToggleVariable("persistent.censorship", True, False)

                    null height gui.pref_spacing + 15

                    vbox:
                        hbox:
                            null width 32

                            label _("Оповещения об интерактиве")
                            textbutton "?":
                                style "pref_description_button"
                                action ShowMenu(
                                    "extra_screen_help",
                                    _("ОПОВЕЩЕНИЯ ОБ ИНТЕРАКТИВЕ\nОповещения об интерактиве включают в себя дополнительные всплывающие подсказки\nо том, какой вид интерактивного момента начнётся по сюжету в скором времени,\nпотому что игрок может не подразумевать о его начале.\nДля активации режима нажмите на кнопку \"Активировать оповещения\".\nЕсли она горит зелёным цветом -- предупреждения активированы."),
                                    ok_action = Hide("extra_screen_help"),
                                    chibi = "y_chibi turned magnifier",
                                    chibi_pos = 30
                                )
                        ...
                        textbutton _("Активировать оповещения"):
                            action ToggleVariable("persistent.warning_interactive", True, False)
...
screen keyboard_help():
    ...
    vbox:
        ...
        if persistent.streamer_detected:
            hbox:
                label "H"
                text _("ДЛЯ СТРИМЕРОВ: скрывает игровой интерфейс и интерфейс телефона для создания превью.\n\nВАЖНО: скрывается не только интерфейс, но и все активные «экраны» (на них отображаются элементы «интерактивных» моментов сюжета с прожатием кнопок клавиатуры или областей на самом экране), поэтому пользуйтесь этой кнопкой только в моменты отсутствия «интерактива», чтобы не испортить впечатление от прохождения (если всё-таки это случилось, то просто прожмите кнопку повторно, чтобы вернуть отображение «экранов»). Подробнее об «интерактиве» можно узнать в разделе «Особенности мода».\n\n{b}Кнопка становится активной только СО второго захода в игру, поэтому рекомендуется перезайти в игру, если Вы ещё этого не сделали{/b}")
    ...
    vbox:
        ...
        if config.developer:
            ...
            hbox:
                ...
                text "Альтернатива Page Up (откат на прошлый текст сюжета на столько, на сколько прокручено само колесо)."

            hbox:
                label "Прокрутка колеса мыши вниз"
                text "Альтернатива Page Down и обычному пропуску (пропускает текст сюжета на столько, на сколько прокручено само колесо)."
...
screen feature_help():
    ...
    vbox:
        ...
        hbox:
            ...
            text _("В начале каждого дня (и не только) перед первой фразой будет создано автоматическое сохранение. Загрузить его можно в меню слотов сохранений из специального раздела, помеченного буквой «A».")
