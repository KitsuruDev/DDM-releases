...
label monika_first_run:
    ...
    label monika_first_run_create_name:
        ...
        if playername_raw == 0:
            ...
            if count_swearing < 5:
                ...
                m "Можешь обосраться от счастья или побежать выёбываться в своём микросоциуме скриншотом, ржа над моей реакцией, вместо того, чтобы проанализировать свои изъяны и попытаться их хоть как-то исправить."
    ...
    pause 0.25
    show monika mh
    m "{i}Если Вы являетесь стримером/лестплейщиком или просто снимаете видео по данному моду, обязательно включите режим стримера в настройках.{/i}"
    m "{i}И обязательно ознакомьтесь с разделом «Помощь» в меню, где расписаны все горячие клавиши и нюансы данного мода.{/i}"
    m "{i}А ещё учтите, что в данной модификации поднимаются различные темы, связанные с людскими пороками, психологическими проблемами и так далее.{/i}"
    m "{i}И в некоторых моментах сюжета они будут подаваться в «грубой» и «броской» форме, что может вызвать дискомфорт при прохождении.{/i}"
    m "{i}Поэтому будьте к этому эмоционально готовы.{/i}"
    show monika cm
    show monika ldown -planchette with dissolve
    pause 0.25
    show monika om oe
