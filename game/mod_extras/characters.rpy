...
init -1 python in Characters:
    ...
    class CharactersImage:
        def __init__(self, name, age, gender, color, name_color, nationality, hobby, status, description, sprites={}, cut=36, zoom_profile=0.5):
            self.name = name
            self.profile = _(
                    "Имя: %(name)s.\nВозраст: %(age)s.\nПол: %(gender)s.\nЦвет имени: {color=%(color)s}%(name_color)s{/color}.\nНациональность: %(nationality)s.\nУвлечения: %(hobby)s.\nСтатус: %(status)s."
                ) % {
                    "name": name,
                    "age": age,
                    "gender": gender,
...
тщательно проанализировав всю сложившуюся ситуцию. И, кажется, у него началась новая жизнь. Станет ли она счастливее прошлой? Это уже зависит \
...
трудности, из-за которых он балансировал на грани развала, она нашла несколько человек на постоянной основе, с которыми крепко подружились. \
После вступления в клуб Макса быстро в него влюбилась, и уже через неделю он ответил ей взаимностью, подойдя к любовному выбору со всей \
серьёзностью. Казалось бы, жизнь Моники «удалась», но в ней есть единственная, но важная проблема -- это отношения со строгими родителями, \
которые хотят вырастить дочь такой, какой они её видят. Однако если Моника справилась с опекой Литературного клуба, то и здесь справится на \
«максимальный балл». Правда же?
    """),
    sprites = {
    # Школьная форма
...
позитивного настроения. Но иногда её особенность может выйти из-под контроля, обернувшись для неё злой шуткой. Макс, как её близкий друг, очень \
...
не рассчитывает из-за больного опыта доверия к окружающим.
    """),
    sprites = {
    # Школьная форма
...
подружился с Нацуки, однако из-за известной только им причины она его сильно возненавидела, но Хироши из-за своей влюблённости не собирался \
...
учесть, что у него ещё хороший заработок и приятная работа, то можно с уверенностью сказать, что жизнь Мартина точно удалась.
    """),
    sprites = {
    # Рабочий стиль
