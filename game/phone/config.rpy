...
python early in phone.config:
    ...
    # Transitions
    enter_transition = Dissolve(0.6, time_warp=_warper.ease) # showing phone
    exit_transition = Dissolve(0.6, time_warp=_warper.ease) # hiding phone
    ...
    # A set of text tags that are allowed in text messages. They should not change the size of the text.
    message_text_tags = { "emoji", "alpha", "color", "u", "i", "b", "a" }
    ...
    hide_status_bar_screens = [
        "phone_call", "phone_call_accept",
        "phone_game_snake", "phone_game_snake_record", "phone_game_snake_round", "phone_game_snake_before_after_round"
    ]
    ...
    layer_at_transforms = { } # `None` is set later

    # How many "pages" of application we use in the `phone` screen.
    applications_pages = 1
    ...
    data = {"call_history": list, "group_chats": list, "background_image": lambda: None, "calendars": list}

    def _generate_applications_dict():
        rv = { i: phone.application._generate_applications_page() for i in range(applications_pages) }
    ...
    #               -For a date, a tuple of (`day`, `month`, `year`, `hours`, `minutes`, `seconds`).
