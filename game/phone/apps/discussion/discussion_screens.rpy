    ...
    use _phone():
        side "t b c":
            use app_base(action=(SetField(phone.discussion, "_group_chat", None), Function(phone.discussion.audio_messages.reset))):
                ...
                hbox:
                    if has_group_chat:
                        add phone.discussion._group_chat.icon at _fits(50) yalign 0.5
    ...
    frame:
        side "l r":
            if phone.discussion._current_payload is not None and phone.discussion._current_payload.source is not None:
                ...
                if sender.is_pov and phone.discussion._current_payload.type == phone.discussion._PayloadTypes.TEXT:
                    fixed style "empty":
                        yfill True
                        xsize 0.875
                    ...
                    text _("Сообщение") color "#666"
            else:
                text _("Сообщение") color "#666"

            text _("Отправка") color "#0094FF" xalign 1.0
...
screen _chat_message(p):
    ...
    hbox:
        if sender.is_pov:
            ...
            add sender.icon at _fits(45) yalign 0.0
        ...
        frame style "empty":
            ...
            vbox:
                ...
                if phone.discussion._group_chat._can_load_more():
                    textbutton _("Загрузить больше сообщений"):
                        action (
