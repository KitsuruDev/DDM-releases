init -100 python in phone.discussion:
    ...
    def end_discussion(transition = False):
        store._window_hide()
    ...
    def _run_callbacks(gc, event, payload):
        ...
        callback_object.data = None if payload.type == VIDEO else payload.data
    ...
    def _register_date(group, day, month, year, hour, minute, second):
        group = group_chat(group)

        if (group.date.year, group.date.month, group.date.day) < (year, month, day):
            group._save_payload(_Payload(None, format_date(day, month, year), DATE), False)
        ...
        _run_callbacks(group, "save", _Payload(None, (day, month, year, hour, minute, second), LABEL))

    def _get_date(group, day, month, year, hour, minute, second, auto):
        group = group_chat(group)
    ...
    def date(day, month, year, hour, minute, second, delay=0.5, auto=False):
        dc = _discussion_coroutine()
        dc.send(None)

        date_tuple = _get_date(_group_chat, day, month, year, hour, minute, second, auto)
    ...
    def register_date(group, day, month, year, hour, minute, second, auto=False):
        _register_date(group, *_get_date(group, day, month, year, hour, minute, second, auto))
...
python early in phone.discussion._PayloadTypes: # fake enum because of the module not existing in python 2.7 (it's a 3.4 thing)
    TYPING, TEXT, IMAGE, LABEL, DATE, MENU, AUDIO, VIDEO = 0, 1, 2, 3, 4, 5, 6, 7
