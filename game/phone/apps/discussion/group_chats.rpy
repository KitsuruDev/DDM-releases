init -100 python in phone.group_chat:
    ...
    short_name_length = 10 # The max lenght of a *group chat*'s name shortened.
    messages_displayed = 175 # How many messages we display at the same time.
    messages_fill_if_lower = 15 # If the next "load" of messages contains this many or less messages, add those messages to the current load.
    ...
    class GroupChat(object):
        ...
        def number_of_messages_sent(self, char):
            key = charcter(char).key if char is not None else char
            rv = 0
        ...
        def _get_display_last_message(self):
            ...
            if not self._payloads:
                ...
                if _type == _PayloadTypes.TEXT: message, italic = renpy.substitute(p.data), False
                elif _type == _PayloadTypes.IMAGE: message = __("Медиа-файл отправлен")
                elif _type == _PayloadTypes.LABEL: message = renpy.substitute(p.data)
                elif _type == _PayloadTypes.DATE: message = p.data
                elif _type == _PayloadTypes.AUDIO: message = __("Аудио-файл отправлен")
                elif _type == _PayloadTypes.VIDEO: message = __("Медиа-файл отправлен")

            message = remove_text_tags(message)

            LIMIT = 27
            if len(message) >= LIMIT: message = message[:LIMIT - 3] + "..."
        ...
        def _date_text(self):
            date = self.date
            return format_date(date.day, date.month, date.year)
