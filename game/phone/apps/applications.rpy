init -100 python in phone.application:
    ...
    from store import Gradient, RoundedCorners, Transform, Text, phone, NullAction, Fixed, gui, Null, Flatten
    ...
    def IconNull(background):
        bg = renpy.easy.displayable_or_none(background)
        rv = Fixed(bg, style="empty", xysize=(gui.phone_application_icon_size, gui.phone_application_icon_size))
        return Flatten(rv)
    ...
    def move_application(start, end, key=None):
        ...
        start_app_page, end_app_page = get_app_page(start_page, key), get_app_page(end_page, key)
...
screen phone():
    ...
    python:
        max_page = min(phone.application.get_app_dict(None)["max"] + (coords_to_move is not None), phone.config.applications_pages - 1)
    ...
    use _phone():
        ...
        add (phone.data[store.pov_key_mobile]["background_image"] or Gradient("#0a8be7", "#32F5EE")):
            at transform:
    ...
    fixed:
        if app is None:
            null:
                ...
                height (gui.phone_application_icon_size + style._phone_application_button_vbox.spacing +
                    style._phone_application_button_text.size + 1)
        else:
            button at _phone_application_button:
                vbox:
                    add app.icon at _fits(None)
                    if app.name == "Календарь":
                        text phone.calendar.current_day[1]:
                            style "empty" color "#f00"
                            size 16 text_align 0.5 xalign 0.5 yoffset -64
                            font phone.config.basedir + "fonts/tahoma.ttf"
                        text str(phone.calendar.current_day[0]):
                            style "empty" color "#141414"
                            size 40 text_align 0.5 xalign 0.5 yoffset -69
                            font phone.config.basedir + "fonts/Metropolis-Regular.otf"
                        text app.name:
                            yoffset -68
                    else:
                        text app.name
...
style _phone_application_button_text is empty:
    ...
    outlines [ ] color "#000"
    size 12 font phone.config.basedir + "fonts/tahoma.ttf"
    line_spacing 0

style _phone_application_button_fixed is empty:
    fit_first True

init python in phone.application:
    from store import PhoneMenu, ShowMenu, TintMatrix

    app_message = Application(
        _("Мессенджер"),
        Icon(config.basedir + "message_icon.png", 0.9, GradientBackground("#5bf676", "#04be25")),
        PhoneMenu("phone_contacts")
    )

    app_calendar = Application(
        _("Календарь"),
        IconNull(background=IconBackground("#f5f5f5")),
        PhoneMenu("phone_calendars")
    )

    app_call_history = Application(
        _("Телефон"),
        Icon(config.basedir + "call_icon.png", 0.9, GradientBackground("#5bf676", "#04be25")),
        PhoneMenu("phone_call_history")
    )

    app_game_snake = Application(
        _("Змейка"),
        Icon(config.basedir + "game_snake_icon.png", 0.9, GradientBackground("#5bf676", "#04be25")),
        PhoneMenu("phone_game_snake")
    )
