...
python early in phone._lint:
    ...
    def error(msg):
        renpy.error(f"Better EMR Phone - {msg}")
    ...
    def group_chat(gc, log=True):
        ...
        if gc is not cant_evaluate:
            try:
                ...
                error(f"'{gc}' is not a *group chat*.")
...
python early in phone:
        ...
        def lint(self):
            ...
            if month is not _lint.cant_evaluate:
                if month not in (None, True) and (not isinstance(month, (int, float)) or not 1 <= month <= 12):
                    _lint.error(f"'{month}' isn't a valid month.")
            ...
            if day is not _lint.cant_evaluate:
                if day not in (None, True) and (not isinstance(day, (int, float)) or not 1 <= day <= 31):
                    _lint.error(f"'{day}' isn't a valid day.")
            ...
            if minute is not _lint.cant_evaluate:
                if minute not in (None, True) and (not isinstance(minute, (int, float)) or not 0 <= minute <= 59):
                    _lint.error(f"'{minute}' isn't a valid minute.")
            ...
            if hour is not _lint.cant_evaluate:
                if hour not in (None, True) and (not isinstance(hour, (int, float)) or not 0 <= hour <= 23):
                    _lint.error(f"'{hour}' isn't a valid hour.")
            ...
            if second is not _lint.cant_evaluate:
                if second not in (None, True) and (not isinstance(second, (int, float)) or not 0 <= second <= 59):
                    _lint.error(f"'{second}' isn't a valid second.")
    ...
    def _parse_phone_image(ll, register):
        ...
        while not ll.eol():
            ...
            if kwarg == "time":
                ...
                ll.revert(state)
                ll.error(f"unknown property '{kwarg}'")
    ...
    def _parse_phone_date(ll, register):
        kwargs = {time_thing: "None" for time_thing in ("day", "month", "year", "hour", "minute", "second")}
        kwargs["auto"] = "False"
        seen = set()
        ...
        while True:
            ...
            if t not in kwargs:
                ll.revert(state)
                ll.error(f"'{t}' isn't a valid property for the 'time' statement")

            if t in seen:
                ll.revert(state)
                ll.error(f"'{t}' already given")
    ...
    def _parse_phone_if(ll, register): # literally renpy.parser.if_statement
        ...
        return _RawPhoneRegisterIf(entries) if register else _RawPhoneIf(entries)
    ...
    def _get_phone_statements(ll, discussion):
        ...
        if discussion:
            while ll.advance():
                if ll.keyword("image"):    statement = _parse_phone_image(ll, False)
                elif ll.keyword("label"):  statement = _parse_phone_label(ll, False)
                elif ll.keyword("time"):   statement = _parse_phone_date(ll, False)
                elif ll.keyword("type"):   statement = _parse_phone_typing(ll)
                elif ll.keyword("if"):     statement = _parse_phone_if(ll, False)
                elif ll.keyword("menu"):   statement = _parse_phone_menu(ll)
                elif ll.match("\$"):       statement = _parse_phone_one_line_python(ll)
                elif ll.keyword("python"): statement = _prase_phone_python(ll)
                elif ll.keyword("audio"):  statement = _parse_phone_audio(ll, False)
                elif ll.keyword("pass"):   statement = _RawPhonePass()
                elif ll.keyword("pause"):  statement = _parse_phone_pause(ll)
                else: statement = _parse_phone_message(ll, False)
            ...
            while ll.advance():
                if ll.keyword("image"):    statement = _parse_phone_image(ll, True)
                elif ll.keyword("label"):  statement = _parse_phone_label(ll, True)
                elif ll.keyword("time"):   statement = _parse_phone_date(ll, True)
                elif ll.keyword("if"):     statement = _parse_phone_if(ll, True)
                elif ll.match("\$"):       statement = _parse_phone_one_line_python(ll)
                elif ll.keyword("python"): statement = _prase_phone_python(ll)
                elif ll.keyword("audio"):  statement = _parse_phone_audio(ll, True)
                elif ll.keyword("pass"):   statement = _RawPhoneRegisterPass()
                else: statement = _parse_phone_message(ll, True)
    ...
    def _parse_phone_end_discussion(l):
        transition = bool(l.keyword("transition"))
        l.expect_eol()
        l.expect_noblock("phone end discussion")
        return transition

    def _execute_phone_end_discussion(rv):
        discussion.end_discussion(rv)

    renpy.register_statement(
        "phone end discussion",
        parse=_parse_phone_end_discussion, # cds_utils.null_parser
        execute=_execute_phone_end_discussion
    )

    ########################################################

    def _parse_phone_call_accept(l):
        rv = l.require(l.simple_expression)
        l.expect_eol()
        l.expect_noblock("phone call_accept")
        return rv

    def _execute_phone_call_accept(tu):
        c = character.character(eval(tu, store.__dict__))
        calls.call_accept(c)

    def _predict_phone_call_accept(tu):
        renpy.predict_screen("phone_call_accept")
        return [ ]

    def _lint_phone_call_accept(tu):
        _lint.character(tu)

    renpy.register_statement(
        "phone call_accept",
        parse=_parse_phone_call_accept,
        execute=_execute_phone_call_accept,
        predict=_predict_phone_call_accept,
        lint=_lint_phone_call_accept
    )
    ...
    def _predict_phone_call(tu):
        renpy.predict_screen("phone_call", video=tu[2])
        return [ ]
    ...
    def _parse_phone_end_call(l):
        date = l.require(l.simple_expression)
        transition = bool(l.keyword("transition"))
        l.expect_eol()
        l.expect_noblock("phone end call")
        return date, transition

    def _execute_phone_end_call(tu):
        calls.end_call(*tu)

    renpy.register_statement(
        "phone end call",
        parse=_parse_phone_end_call,
    ...
    def _parse_phone_register(l):
        if l.init: # set the pos to the beginning of the statement makes the error message clearer
            l.unadvance()
            l.advance()
    ...
    def _translation_strings_phone_register(rpr):
        ...
        def get_strings(l):
            for statement in l:
                if isinstance(statement, _RawPhoneRegisterMessage): rv.append(statement.message)
                elif isinstance(statement, _RawPhoneRegisterLabel): rv.append(statement.label)
                elif isinstance(statement, _RawPhoneRegisterIf): get_strings(block[1] for block in statement.blocks)
    ...
    def _parse_init_phone_register(l):
        ...
        name = key = icon = _as = default_statement = None
        chars, transient = [ ], False
            ...
            while dl.advance():
                while not dl.eol():
                    ...
                    if p == "key":
                        ...
                        if char in chars:
                            dl.error(f"character '{char}' already given")
                        chars.append(char)
                        ...
                        dl.revert(state)
                        dl.error(f"unknown property '{p}'")

            if key is None:
                dl.error("expected 'key' property")

            if icon is None:
                icon = 'phone.config.basedir + "default.png"'
