####### Phone EMR - Elckarow#8399, version (3, 2, 1) #######

python early in phone:
    from renpy import store
    from store import config as renpy_config
...
default -150 phone.data = collections.defaultdict(phone._Data())
...
screen _phone(xpos=0.5, xanchor=0.5, ypos=0.1, yanchor=0.1, horizontal=False):
    frame style "empty" modal True:
        at transform:
            subpixel True
            zoom gui.phone_zoom * (1.3 if horizontal else 1.0)
            pos (xpos, ypos) anchor (xanchor, yanchor)

        background Transform(
            phone.config.basedir + "background/background_" + pov_key_mobile + ".png",
            subpixel=True,
            align=(0.5, 0.5),
        ...
        fixed style "empty":
            ...
            # the thing just above about config.at_transform_compare_full_context
            $ alpha = 1.0 - phone.system.brightness
            add "#000":
                at transform:
                    alpha alpha

    on "hide" action SetVariable("phone.system.at_list", [])
...
init 1500:
    # narrator is guarenteed to exist at init 1400, see renpy/common/00definitions.rpy
    ...
    python in phone:
        ...
        renpy.add_layer("call_layer", above = "screens", menu_clear = True)
        renpy.add_layer("call_layer_gui", above = "call_layer", menu_clear = True)
        renpy_config.layer_clipping["call_layer"] = (103, 75, 304, 534)
