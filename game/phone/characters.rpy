init -100 python in phone.character:
    ...
    from store import Color, __, RoundedCorners
    from store.phone import config

    character_short_name_length = 10 # The max lenght of a *character*'s name shortened.
    message_delay = 1.75 # A number of seconds added to the pause before each message.
    ...
    class Character(object):
        ...
        @property
        def is_pov(self):
            return self.key == store.pov_key_mobile
    ...
    def character(x):
        ...
        if x is None: x = store.pov_key_mobile
